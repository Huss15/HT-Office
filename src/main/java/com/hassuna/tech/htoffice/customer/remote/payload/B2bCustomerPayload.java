...
public record B2bCustomerPayload(String customId, String companyName, String taxId, String VatIdentificationNumber,
                                 AddressPayload address, ContactDataPayload contactData) {
    
    public static B2bCustomerPayload convertToB2bCustomerPayload(B2bCustomer customer) {
        return new B2bCustomerPayload(customer.getCustomId(), customer.getCompanyName(),
                MaskingUtils.maskExceptLast3(customer.getTaxId()), MaskingUtils.maskExceptLast3(customer.getVatIdentificationNumber()),
